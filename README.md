...
- [x] waker page 保序和锁的竞争做一个权衡，需要测试用户调度频率、zCore 协程执行时间等：经过权衡选取了不保序的 bitmap。测试过对 FIFO 队列然后每次中断降低当前时间片执行完毕任务的优先级，但在性能测例上表现有所下降，究其原因在于保序需要对相关数据结构加锁，加锁的开销比保序可能取得的收益要高
- [x] 区分用户任务和纯内核任务，把用户程序当成线程，延迟敏感单独处理：对延迟敏感的用户，异步 syscall 可能不满足其要求，用户可以使用普通的同步 syscall，也可以使用高优先级的异步 syscall。当内核调度识别到高优先级 syscall 后，发生中断会将正在执行的协程退化为线程（因为一个协程的退出至少需要等到下一个退出点），新创建一个线程并立即执行高优先级事件，处理完毕之后回归到之前协程的执行
- [x] 只要时钟中断就 yield 不科学，因为不确定本任务执行了多久：时间片足够短就可以切回来
- [x] 多核，专核专用，动态调整，一个核一个 runtime，利用 shared memory 通信：没有写死某个核是专门做内核任务的，但是是根据任务窃取来动态进行的，执行速度快的内核协程，会被倾向于集中在某个核上，context switch 尽管次数多，但开销较小。而对于其余跑用户程序的核，由于异步系统调用，用户程序只会在时间片耗尽时才会切换到其他进程，切换次数很少
- [x] 编写测试用例：多核调度测例 coretest、集成测例 libctest 和适配在 zCore 上的 lmbench
...
- 支持动态输出在哪个核上跑的任务
- 最后经过权衡选取了不保序的 bitmap 作为记录调度的数据结构。测试过上述 FIFO 队列然后每次中断降低当前时间片执行完毕任务的优先级，但在性能测例上表现有所下降，究其原因在于保序需要对相关数据结构加锁，加锁的开销比保序可能取得的收益要高，保留的优先级仅用来做高优先级的异步 syscall。当内核调度识别到高优先级 syscall 后，发生中断会将正在执行的协程退化为线程（因为一个协程的退出至少需要等到下一个退出点），新创建一个线程并立即执行高优先级事件，处理完毕之后回归到之前协程的执行
...
- 对上周更新后的单核/多核调度器和主线 zCore 做适配，目前已经通过 CI
  - [单核调度器](https://github.com/OSLab-zCore/zCore/actions/runs/2426925844)
  - [多核调度器](https://github.com/OSLab-zCore/zCore/runs/6706676130?check_suite_focus=true)

- 在集成测例的过程中，修复了原用户库中系统调用与 zCore 规范不一致的问题，将目前更新后的用户库抽出来新建仓库 [zcore-user](https://github.com/OSLab-zCore/zcore-user) 并撰写说明文档，保留了大部分常用的系统调用接口（符合 zCore 规范），给之后撰写用户测例提供便利
- 在 Qemu 上跑通集成 libc-test 测例（单核/多核调度器），和刘松铭一起在 U740 上跑通集成 libc-test 测例（全部通过），相关问题修复在 zCore [u740](https://github.com/OSLab-zCore/zCore/tree/u740) 分支
...
- 多核调度器的正确性测试——测例
- 多核调度器的性能测试
  - 包含进程、线程、协程多个级别，不同级别间有通信和交互
  - 与Linux对比
- DEMO录制——正确性测试、性能测试的过程
- 文档完善（可能要包含后人可拓展的目标，例如完善串口驱动和拓展16G页表等）
- PPT准备，测试数据汇总
