...
整体进度大致分为两个阶段：第一阶段的工作是完善zCore在多线程方面的支持（包括锁和信号量等），为后面的工作打下基础；第二阶段的工作分为两个部分，第一个部分是设计多核异步调度器，并修复zCore在多核方面的bug；第二部分是将zCore多核在U740上跑起来（适配U740，硬件相关），并修复一些平台相关的bug。
...
刘松铭：跟随前人的工作在U740起zCore多核，但目前还有bug，猜测bug是由zCore主线更新引起的。原zCore起来的版本是一年前的版本。目前做了一些向新版本的适配，还在debug中。进度记录在[zCore多核适配U740文档](./zCore多核适配U740文档.md)。
...
刘松铭：利用假期几天时间尝试了几种制作SD卡的办法，但是都没有成功。进度记录在[制作SD卡流程文档](./制作SD卡流程文档.md)。目前问题还是无法恢复SD卡之前的数据，接下来会考虑如下的解决方案：
...
1. SD制作的文档记录在[制作SD卡流程文档](制作SD卡流程文档.md).
2. 网络起zCore的方法记录在[zCore多核适配U740文档](zCore多核适配U740文档.md).
...
刘松铭：已经完成串口驱动的编写，参考[U740的datasheet](https://sifive.cdn.prismic.io/sifive/1a82e600-1f93-4f41-b2d8-86ed8b16acba_fu740-c000-manual-v1p6.pdf)。在调试串口的过程中遇到了一个隐秘的bug：正常的plic的memory map排布是每个核M态和S态对应的中断交替，但是U740的小核（0号核）没有S态，因此跳过了0号核的S态。这导致后面的顺序全部差了一个offset，而zCore没有针对这点适配，造成plic enable失败，进而收不到外设中断。解决方法是条件编译判断，增加对应的offset。
